                         ...
                         sparsethr=0.995)
{
    ...
    if(stem) {
        ## Stemming
        skills <- tm_map(skills, content_transformer(stemDocument))
        #skills[[1]]
    }
    ...
    cl <- bigkmeans(as.big.matrix(m), nocluster)
                               ...
                               nmethod="qgram",...)
{
    ...
    ## only works as dataframes
    s.df <- data.frame(s.df)
    lk.df <- data.frame(lk.df)
...
#         }
    ...
    ### Valid Names in lowercase - remove duplicate "."s
    ###setnames(fdt,namesv,gsub("\\.+",".",tolower(make.names(namesv,unique=T,allow=F))))
    namesn <- gsub("\\.+",".",tolower(make.names(namesv,unique=T,allow=F)))
    setnames(fdt,namesv,namesn)
...
#################################################################
df.get.lkup.val <- function(sdf,ldf,scol,lcol,sassigncol,lassigncol,nomatch)
{
   sdf[,sassigncol] <- ldf[match(sdf[,scol],ldf[,lcol]),lassigncol]
   findna <- is.na(sdf[,sassigncol])
   sdf[findna,sassigncol] <- nomatch
   return(sdf)
}
